...
'''
I have completed the preprocessing part for all the 4 datasets as Madhava had mentioned in the meeting. The steps involved in preprocessing were -
1. Checking and removing null values
2. Visualizing the data to view what kind of distribution do we have
3. Removing outliers using Z-Score
4. Perfomed feature selection using XGBoost (The number of features were reduced to half)
5. Scaling the features the data using Standard Scaler
6. Handling the class imbalance problem using SMOTE (Synthetic Minority Oversampling Technique)

The results obtained after preprocessing showed a significant improvement in the test metrics of the model. The results are as follows -
1. For ER Subtype the maximum accuracy, precision and recall was increased to ~75% - obtd by SVM (75%) and XGBoost (73%)
2. For HER2 Subtype the maximum accuracy, precision and recall was increased to ~83%% - obtd by SVM (83%) and XGBoost (79%)
3. For PR Subtype the maximum accuracy, precision and recall was increased to ~69%% - obtd by SVM (68%) and Random Forest (69%)
4. For Mol Subtype the maximum accuracy, precision and recall was increased to ~82%% - obtd by SVM (82%) and XGBoost (63%)
'''

def get_total_features(label_path='Data/Patient class labels.csv', feature_path='Data/pyradiomics_extraction_box_with_correct_mask.csv', sequence='pre'):
...
def start():
    ...
    report, knn_her2 = get_classification_report(knn_pr, train_x, train_y_her, val_x, val_y_her, test_x, test_y_her, ['HER2 Negative', 'HER2 Positive'], 'HER2 Subtype metrics on testing set')
    result = pd.DataFrame(report, index = [0])
    result['Algorithm'] = 'KNN'
    result['Subtype'] = 'HER2'
    result = result[list(result.columns[-2:]) + list(result.columns[:-2])]
